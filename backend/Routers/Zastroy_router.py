...
def create_new_zastroy(zastroy: ZastroyModel):
    # db: Session = Depends(get_db)
    try:
        # return create_zastroy(db, zastroy)
        return {"message": "Застройщик создан (режим без БД)"}
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))

@router.post("/login")
def zastroy_login(credentials: ZastroyLogin):
    # db: Session = Depends(get_db)
...
def read_zastroys(skip: int = 0, limit: int = 100):
    # db: Session = Depends(get_db)
    # return get_zastroys(db, skip=skip, limit=limit)
    return [{"message": "Список застройщиков (режим без БД)"}]

@router.get("/{zastroy_id}", response_model=ZastroyResponse)
def read_zastroy(zastroy_id: int):
    # db: Session = Depends(get_db)
    # db_zastroy = get_zastroy(db, zastroy_id)
    # if not db_zastroy:
    #     raise HTTPException(status_code=404, detail="Застройщик не найден")
    # return db_zastroy
    return {"message": f"Застройщик {zastroy_id} (режим без БД)"}

@router.put("/{zastroy_id}", response_model=ZastroyResponse)
def update_existing_zastroy(zastroy_id: int, zastroy: ZastroyModel):
    # db: Session = Depends(get_db)
    try:
        # return update_zastroy(db, zastroy_id, zastroy)
        return {"message": f"Застройщик {zastroy_id} обновлен (режим без БД)"}
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))

@router.delete("/{zastroy_id}")
def remove_zastroy(zastroy_id: int):
    # db: Session = Depends(get_db)
    try:
        # return delete_zastroy(db, zastroy_id)
        return {"message": f"Застройщик {zastroy_id} удален (режим без БД)"}
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))